...
.. code-block:: python
	...
	def loadLibrary(location):
	
		#defining variables and functions as 'global' so that they are available outside of the loadLibrary(location) function scope
		global shared_lib_path 
		global QB_LIB
		shared_lib_path = location
		...
		QB_LIB.setLibraryPath(shared_lib_path.encode('utf-8')) #setting the library Path, so that the Library knows about its location!
