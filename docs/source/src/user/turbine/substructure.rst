...
Buoyancy Modeling of SUBMEMBERS
###############################

QBlade evaluates explicit buoyancy by integrating the pressure over the actually wetted outer surfaces of each Morison member, rather than by applying a single lumped buoyancy force at the center of displaced volume. This allows the resulting buoyancy forces, centers of pressure and restoring moments to vary continuously with member orientation, wetted geometry and partial submergence.

The explicit buoyancy calculation is affected by the keywords ``STATICBUOYANCY``, ``BUOYANCYTUNER``, ``BUOYANCYPREDICTOR`` and ``ADVANCEDBUOYANCY``.


:code:`STATICBUOYANCY`
 [bool] Controls the hydrostatic reference level used for the explicit buoyancy calculation.

 If set to ``true``, the explicit buoyancy calculation is referenced to the **mean still water level (MSL)**. In this case, only the MSL-based hydrostatic contribution is included.

 If set to ``false``, the explicit buoyancy calculation on the **side surfaces** is referenced to the **local instantaneous wave elevation**. In this case, the side-surface pressure contains the MSL-based hydrostatic contribution plus the wave-elevation-induced hydrostatic contribution.

 .. code-block:: console
    :caption: The ``STATICBUOYANCY`` keyword

    false    STATICBUOYANCY


:code:`BUOYANCYTUNER`
 [-] Multiplication factor affecting the explicit buoyancy forces. Buoyancy contributions originating from the linear hydrodynamic stiffness matrix are not affected by this factor.

 .. code-block:: console
    :caption: The ``BUOYANCYTUNER`` keyword

    1.0    BUOYANCYTUNER


:code:`BUOYANCYPREDICTOR`
 [-] Dimensionless look-ahead factor for evaluating explicit buoyancy within a time step. Member positions are predicted using velocity and acceleration before computing the wetted geometry and hydrostatic pressure. The resulting buoyancy forces and moments are then applied to the current structural configuration.

 A value of ``0`` disables the predictor. Values between ``0`` and ``1`` can improve robustness for moving members close to the free surface.

 .. code-block:: console
    :caption: The ``BUOYANCYPREDICTOR`` keyword

    0.65    BUOYANCYPREDICTOR


:code:`ADVANCEDBUOYANCY`
 [-] Controls the circumferential discretization of cylindrical members during explicit buoyancy calculations. The sidewalls of a cylindrical member are subdivided into this number of strips for the pressure integration. Larger values increase the geometric resolution for inclined or partially submerged members, at the cost of additional computation.

 .. code-block:: console
    :caption: The ``ADVANCEDBUOYANCY`` keyword
...
The explicit buoyancy formulation distinguishes between **side surfaces** and **axial end faces** and explicitly accounts for partial submergence. The integrated surface-pressure loads are converted into equivalent nodal forces and moments, including the moment generated by the offset of the center of pressure.

The buoyancy load magnitude is evaluated on a **predicted member position**, while the resulting forces and moments are applied to the **current structural configuration**. This improves robustness for moving members, especially close to the free surface.

For partially submerged members, an additional buoyancy-gradient contribution is introduced near the waterline in order to smooth the transition when a member enters or leaves the water.


Hydrostatic Pressure Decomposition
----------------------------------

Using the vertical coordinate :math:`z` relative to MSL (positive upwards) and the local instantaneous wave elevation :math:`\eta`, QBlade evaluates a pressure head :math:`h` on the wetted surface and converts it to pressure through

.. math::

   p = \rho g h

The resulting force is then obtained by integrating the pressure over the actually wetted area:

.. math::

   \mathbf{F} = \int_{A_\mathrm{wet}} p \, \mathbf{n} \, \mathrm{d}A

where :math:`\rho` is the fluid density, :math:`g` is the gravitational acceleration, :math:`\mathbf{n}` is the local surface normal and :math:`A_\mathrm{wet}` is the actually wetted surface area.

In the implementation, two quantities must be distinguished:

- the **clipping elevation**, which determines the actually wetted geometry,
- the **pressure head**, which determines the pressure magnitude acting on that wetted surface.

This distinction is important, because in some cases the wetted geometry may follow the instantaneous free surface, while the pressure head is still referenced to MSL.


Static Hydrostatics
^^^^^^^^^^^^^^^^^^^

For ``STATICBUOYANCY`` set to ``true``, both the clipping elevation and the pressure head are referenced to the **mean still water level (MSL)**.
...
.. math::
   ...
   p_\mathrm{static} = \rho g (-z)

In this mode, only the **MSL-based hydrostatic contribution** is included, and the wetted geometry is also evaluated relative to MSL.

In other words:

- **clipping** is performed against **MSL**
- **pressure head** is referenced to **MSL**


Dynamic Hydrostatics
^^^^^^^^^^^^^^^^^^^^

For ``STATICBUOYANCY`` set to ``false``, the buoyancy-pressure integration on the **side surfaces** is referenced to the **local instantaneous wave elevation**.

Thus, on the side surfaces:
...
.. math::
   ...
   h_\mathrm{dynamic} = -z + \eta

with corresponding pressure

.. math::

   p_\mathrm{dynamic} = \rho g (-z + \eta)

This can be interpreted as the sum of two contributions:

.. math::

   p_\mathrm{dynamic} = \underbrace{\rho g (-z)}_{\text{MSL hydrostatic pressure}}
   + \underbrace{\rho g \eta}_{\text{wave-elevation-induced hydrostatic pressure}}

Thus, the dynamic option adds a **wave-elevation-induced hydrostatic contribution** to the MSL-based hydrostatic pressure, and the wetted geometry is also updated using the instantaneous local free surface.

In other words, on the side surfaces:

- **clipping** is performed against the **instantaneous local wave elevation**
- **pressure head** is referenced to the **instantaneous local wave elevation**

This additional contribution is still purely hydrostatic. It is **not** the decayed wave-pressure term.


Decayed Wave-Pressure Contribution
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The depth-decayed wave-pressure contribution is handled separately through the Morison pressure terms.

It can be written in the general form

.. math::

   p_\mathrm{dec}(z) = \rho g \, \eta \, \chi(z)

where :math:`\chi(z)` is the depth-decay function. In deep water this reduces to

.. math::

   \chi(z) = e^{kz}

and in finite water depth it is typically of the form

.. math::

   \chi(z) = \frac{\cosh(k(z+h))}{\cosh(kh)}

with wave number :math:`k` and water depth :math:`h`.

This term is not introduced through the buoyancy routine itself, but through the corresponding Morison pressure terms.


End-Face Treatment
^^^^^^^^^^^^^^^^^^

In the current implementation, the dynamic hydrostatic option primarily affects the **side surfaces**.

For **axial end faces**, clipping and pressure head may be treated differently depending on the pressure-model coupling:

- if ``CpAx > 0``, the **end-face clipping** may still follow the actual free-surface geometry, but the buoyancy routine contributes only the **MSL-based baseline part** on that wetted area; the wave-induced end pressure itself is then supplied through the Morison end formulation,
- if ``CpAx = 0``, the end faces are treated as **static only** in the buoyancy routine, i.e. both clipping and pressure head are referenced to **MSL**,
- if an end is vented, no net end-face pressure load is applied.

Thus, dynamic hydrostatics modifies the **side-surface** hydrostatic pressure directly, while dynamic **end-face** pressure is represented through the Morison end model whenever ``CpAx > 0`` is active.

Cylindrical Members
-------------------

For cylindrical members, the buoyancy integration is separated into:

- the cylindrical side surface, and
- the two circular end faces.

The side-surface pressure is integrated over the wetted part of the cylindrical mantle. For inclined or partially submerged members, the wetted side strips are clipped against the selected hydrostatic reference level and integrated individually.

The circular end faces are treated analytically. The implementation distinguishes between fully dry, fully wet and partially submerged end faces. For partially submerged end faces, the resulting pressure force and center of pressure are evaluated from the actually wetted circular segment.


Rectangular Members
-------------------

For rectangular members, the buoyancy integration is performed separately on the six outer faces of the rectangular primitive:

- two axial end faces, and
- four side faces.

Each face is clipped against the selected hydrostatic reference level using the predicted geometry. The wetted polygon is then projected into the local face coordinates and the pressure distribution is integrated over the clipped area to obtain both the face force and the corresponding center of pressure.

This approach supports fully submerged faces, partially submerged faces and arbitrary member orientations.


Coupling with Morison Pressure Terms
------------------------------------

Special care is taken to avoid double counting between explicit buoyancy and Morison pressure terms.

**Side surfaces**

The buoyancy routine provides the hydrostatic surface-pressure contribution on the side surfaces. Dynamic wave-excitation loads on the side surfaces are represented separately through the Morison inertia, Froude-Krylov and drag terms. Consequently, ``STATICBUOYANCY = false`` modifies the **side-surface hydrostatic contribution only**.

**Axial end faces**

The axial end faces are treated separately:

- If ``CpAx > 0``, the wave-induced axial end pressure is provided through the Morison end formulation. In this case, the buoyancy routine contributes only the MSL-based baseline part on the end face, while the end-face clipping may still follow the actual free-surface geometry.
- If ``CpAx = 0``, the end faces are treated as **static only** in the buoyancy routine. In this case, no dynamic hydrostatic end-face pressure is introduced through ``STATICBUOYANCY``.
- If an end is vented, no net end-face pressure load is applied.

Thus, in the current formulation, the dynamic buoyancy option primarily affects the **side surfaces**, while dynamic **end-face** pressure is handled through the Morison end model whenever ``CpAx > 0`` is active.


Coupling with Potential-Flow Excitation
---------------------------------------

When wave excitation is provided through a potential-flow formulation, such as WAMIT, NEMOH or AQWA based databases, the recommended approach is to use:

- ``STATICBUOYANCY`` set to ``true`` for the explicit buoyancy calculation, and
- the external potential-flow model for the wave-excitation forces.

This keeps the explicit buoyancy contribution limited to the MSL-based hydrostatic restoring part and avoids adding an additional wave-elevation-induced hydrostatic contribution through the side-surface buoyancy formulation.
